  ...
  app.get("/api/Topic_2s", function (req, res) {
    ...
    app.get("/api/Topic_2s/:id", function (req, res) {
      ...
      db.Topic_2.findOne({
        ...
        // res.json(dbTopic_2); // based on what I have from sequelizedBurger should look like this:
        ...
        var topic2OutputObject = {
          topic_2: {
            question: dbTopic_2.question,
        ...
        currentCorrectAnswer = dbTopic_2.correctAnswer;
        res.render("index", topic2OutputObject);
      }).catch((err) => {
        res.status(500).json({
          error: err.message
        });
